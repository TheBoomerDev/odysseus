...
def setup_cookbook_routes() -> APIRouter:
    ...
    def _diagnose_serve_output(text: str) -> dict | None:
        ...
        patterns = [
            ...
            (
                ...
                [
                    {
                        "label": "install OS packages: libnuma-dev python3.12-dev build-essential",
                        "op": "manual",
                    },
                    {
                        "label": "upgrade sglang-kernel after OS packages are installed",
                        "op": "manual",
                    },
                ],
            ),
            ...
            (
                ...
                [
                    {
                        "label": "install build deps for llama.cpp (apt: cmake build-essential git / pacman: cmake base-devel git / dnf: cmake gcc-c++ make git / brew: cmake git)",
                        "op": "dependency",
            ...
            (
                ...
                [
                    {
                        "label": "install build deps for llama.cpp on this server",
                        "op": "dependency",
                        "package": "llama-cpp-python[server]",
                    }
                ],
        ...
        if re.search(
            r"Traceback \(most recent call last\)", tail, re.I
        ) and not re.search(
    ...
    def _normalize_minimax_m3_vllm_cmd(cmd: str) -> str:
        ...
        if (
            not cmd
            or "vllm serve" not in cmd_lower
            or "minimax" not in cmd_lower
            or "m3" not in cmd_lower
        ):
            return cmd
        try:
    ...
    async def generate_cookbook_ssh_key(request: Request):
        ...
        if not key_path.exists():
            ...
            if proc.returncode != 0:
                detail = (
                    (stderr or stdout).decode("utf-8", errors="replace").strip()[-500:]
                )
    ...
    def _needs_binary(cmd: str, binary: str) -> bool:
        return bool(
            re.search(rf"(^|[\s;&|()]){re.escape(binary)}($|[\s;&|()])", cmd or "")
        )
    ...
    async def _binary_available(
        ...
        if remote:
            return await _remote_binary_available(
                remote, ssh_port, binary, windows=windows
            )
    ...
    async def model_download(request: Request, req: ModelDownloadRequest):
        ...
        is_ollama_download = backend == "ollama" or (
            "/" not in req.repo_id and ":" in req.repo_id
        )
        ...
        req.hf_token = (
            "" if is_ollama_download else (req.hf_token or _load_stored_hf_token())
        )
        ...
        _dl_hf_home_shell = (
            _shell_path(req.local_dir.rstrip("/")) if req.local_dir else None
        )
        ...
        if is_ollama_download:
            ...
            lines.append(
                "  ODYSSEUS_OLLAMA_CONTAINER=\"$(docker ps --format '{{.Names}}' 2>/dev/null | grep -E '^(ollama-rocm|ollama-test)$' | head -1)\""
            )
            lines.append('  if [ -n "$ODYSSEUS_OLLAMA_CONTAINER" ]; then')
            lines.append(
                f"    ODYSSEUS_OLLAMA_PULL_CMD={shlex.quote('docker exec ${ODYSSEUS_OLLAMA_CONTAINER} ' + ollama_cmd)}"
            )
            lines.append("  fi")
            lines.append("fi")
            lines.append(
                'if [ -z "$ODYSSEUS_OLLAMA_PULL_CMD" ]; then echo "ERROR: Ollama not found on this server. Install Ollama or start an ollama-rocm/ollama-test container."; exit 127; fi'
            )
        else:
            lines.append(
        ...
        if remote and is_windows:
            ...
            if is_ollama_download:
                ps_lines.append(
                    'if (-not (Get-Command ollama -ErrorAction SilentlyContinue)) { Write-Host "ERROR: Ollama not found. Install from https://ollama.com/download/windows"; exit 127 }'
                )
                ps_lines.append(f"$null | ollama pull '{_ps_squote(req.repo_id)}'")
                ps_lines.append(
                    'if ($LASTEXITCODE -eq 0) { Write-Host ""; Write-Host "DOWNLOAD_OK" } else { Write-Host ""; Write-Host "DOWNLOAD_FAILED (exit $LASTEXITCODE)" }'
                )
            else:
            ...
            if _dl_hf_home_shell and not is_ollama_download:
                ...
                runner_lines.append(
                    f"export HUGGINGFACE_HUB_CACHE={_dl_hf_home_shell}/hub"
                )
            ...
            if is_ollama_download:
                ...
                runner_lines.append(
                    f"  ODYSSEUS_OLLAMA_PULL_CMD={shlex.quote(ollama_cmd)}"
                )
            ...
            if is_ollama_download:
                ...
                runner_lines.append(
                    '    [ $_attempt -eq 1 ] && echo "hf CLI not found, using Python huggingface_hub..."'
                )
                runner_lines.append(
                    f"    python3 -c \"import os; from huggingface_hub import snapshot_download; snapshot_download('{req.repo_id}'{_dl_pyarg}, max_workers={mw})\""
                )
                runner_lines.append("  else")
                runner_lines.append(
                    '    echo "Installing huggingface-hub and dependencies..."'
                )
                runner_lines.append(
            ...
            runner_lines.append(
                '    echo ""; echo "Download attempt $_attempt failed (exit $_ec) — retrying in 30s..."'
            )
            runner_lines.append("    sleep 30")
            runner_lines.append("  fi")
            runner_lines.append("done")
            runner_lines.append(
                'if [ $_ec -eq 0 ]; then echo ""; echo "DOWNLOAD_OK"; else echo ""; echo "DOWNLOAD_FAILED (exit $_ec after $_attempt attempts)"; fi'
            )
            ...
            _hf_invoke = (
                'eval "$ODYSSEUS_OLLAMA_PULL_CMD" < /dev/null'
                if is_ollama_download
                else (hf_cmd if IS_WINDOWS else f"{hf_cmd} < /dev/null")
            )
        ...
        logger.info(
            f"Model download: {req.repo_id} (backend={'ollama' if is_ollama_download else 'hf'}, include={req.include}, session={session_id}, remote={remote})"
        )
    ...
    def _auto_register_image_endpoint(
        req: ServeRequest, remote: str | None
    ) -> str | None:
    ...
    async def _serve_crash_watchdog(
        ...
        for wait_s in _waits:
            ...
            if exit_code == 0:
                ...
                logger.info(
                    f"crash-watchdog: serve {session_id} exited cleanly (0); leaving endpoint {endpoint_id}"
                )
                return
            ...
            return
        logger.debug(
            f"crash-watchdog: no exit marker for {session_id} within window; leaving endpoint {endpoint_id}"
        )
    ...
    def _auto_register_llm_endpoint(
        ...
        if remote:
            ...
            r"\bdocker\s+exec\s+(?:ollama-rocm|ollama-test)\b", req.cmd or ""
        ):
        ...
        try:
            ...
            db.commit()
            logger.info(
                f"Auto-registered local model endpoint: {display_name} @ {base_url}"
            )
            ...
            try:
                ...
                if probed:
                    ...
                    db.commit()
                    logger.info(
                        f"Auto-register: probed {len(probed)} models @ {base_url}"
                    )
            except Exception as _pe:
                logger.warning(
                    f"Auto-register: probe-after-create failed for {base_url}: {_pe!r}"
                )
            return ep_id
        except Exception as e:
    ...
    async def model_serve(request: Request, req: ServeRequest):
        ...
        if re.search(r"\bollama\s+serve\b", req.cmd) and "OLLAMA_HOST=" not in req.cmd:
            ...
            if _ollama_chosen_port:
                req.cmd = (
                    f"OLLAMA_HOST={_ollama_bind_host}:{_ollama_chosen_port} {req.cmd}"
                )
        ...
        if is_windows and remote:
            ...
            if not remote:
                ...
                if local_windows:
                    ...
                    runner_lines.append(
                        'export PATH="$HOME/bin:$HOME/llama.cpp/build-cuda/bin/Release:$HOME/llama.cpp/build/bin/Release:$HOME/llama.cpp/build/bin/Debug:$HOME/llama.cpp/build/bin:$PATH"'
                    )
            ...
            local_windows_llama_cmd = local_windows and (
                "llama_cpp" in req.cmd or "llama-server" in req.cmd
            )
            if (
                "llama_cpp" in req.cmd or "llama-server" in req.cmd
            ) and not local_windows_llama_cmd:
                ...
                runner_lines.append(
                    "# Ensure a llama.cpp server (prefer native llama-server)"
                )
                ...
                runner_lines.append(
                    "  # Termux: no native build — use the Python bindings (CPU)."
                )
                runner_lines.append(
                ...
                runner_lines.append(
                    "    pip install numpy diskcache jinja2 2>/dev/null"
                )
                runner_lines.append(
                ...
                runner_lines.append(
                    "  [ -r ~/.config/odysseus-llama-cpp-env ] && . ~/.config/odysseus-llama-cpp-env"
                )
                ...
                runner_lines.append(
                    '  if command -v nvidia-smi >/dev/null 2>&1 && nvidia-smi -L 2>/dev/null | grep -q "GPU " && python3 -c "import llama_cpp" 2>/dev/null; then'
                )
                runner_lines.append(
                    '    if ! python3 -c "import llama_cpp; import sys; sys.exit(0 if llama_cpp.llama_supports_gpu_offload() else 1)" 2>/dev/null; then'
                )
                runner_lines.append(
                    '      echo "[odysseus] NVIDIA detected but installed llama-cpp-python is CPU-only — reinstalling with CUDA wheel index for GPU offload..."'
                )
                runner_lines.append(
                ...
                runner_lines.append(
                    '        echo "[odysseus] llama-cpp-python now supports GPU offload."'
                )
                runner_lines.append("      fi")
                runner_lines.append("    fi")
                runner_lines.append("  fi")
                ...
                runner_lines.append(
                    '  if ! command -v llama-server >/dev/null 2>&1 && python3 -c "import llama_cpp" 2>/dev/null; then'
                )
                runner_lines.append("    mkdir -p ~/bin")
                runner_lines.append(
                    "    cat > ~/bin/llama-server <<'_ODY_LLAMA_SHIM_EOF'"
                )
                runner_lines.append("#!/usr/bin/env bash")
                runner_lines.append(
                    "# Auto-generated by Odysseus Cookbook: a `llama-server` lookalike"
                )
                runner_lines.append(
                    "# that translates the native CLI to `python -m llama_cpp.server`."
                )
                runner_lines.append(
                    "# Lets cookbook-generated launch commands run unchanged on hosts"
                )
                runner_lines.append(
                    "# where only the pip llama-cpp-python package is installed."
                )
                runner_lines.append("ARGS=()")
                runner_lines.append("while [ $# -gt 0 ]; do")
                runner_lines.append('  case "$1" in')
                runner_lines.append(
                    '    -ngl|--gpu-layers|--n-gpu-layers) ARGS+=(--n_gpu_layers "$2"); shift 2 ;;'
                )
                runner_lines.append(
                    '    -c|--ctx-size) ARGS+=(--n_ctx "$2"); shift 2 ;;'
                )
                runner_lines.append(
                    '    -b|--batch-size) ARGS+=(--n_batch "$2"); shift 2 ;;'
                )
                runner_lines.append(
                    "    -ub|--ubatch-size) shift 2 ;;  # llama-cpp-python has no separate ubatch"
                )
                runner_lines.append(
                    "    --flash-attn) ARGS+=(--flash_attn true); shift 2 ;;"
                )
                runner_lines.append(
                    '    --cache-type-k) ARGS+=(--type_k "$2"); shift 2 ;;'
                )
                runner_lines.append(
                    '    --cache-type-v) ARGS+=(--type_v "$2"); shift 2 ;;'
                )
                runner_lines.append(
                    '    --n-cpu-moe) ARGS+=(--n_cpu_moe "$2"); shift 2 ;;'
                )
                runner_lines.append(
                    '    --mmproj) ARGS+=(--clip_model_path "$2"); shift 2 ;;'
                )
                ...
                runner_lines.append(
                    '    --chat-template) ARGS+=(--chat_format "$2"); shift 2 ;;'
                )
                runner_lines.append(
                    "    --fit|--split-mode|--tensor-split|--main-gpu|--parallel) shift 2 ;;  # native-only"
                )
                ...
                runner_lines.append(
                    '    echo "[odysseus] Created llama-server shim → python -m llama_cpp.server (no native binary needed)"'
                )
                runner_lines.append("  fi")
                runner_lines.append(
                ...
                runner_lines.append(
                    '  ODYSSEUS_VLLM_HELP_CMD="$(python3 - "$ODYSSEUS_SERVE_CMD" <<\'PY\''
                )
                ...
                runner_lines.append(
                    '  if eval "$ODYSSEUS_VLLM_HELP_CMD" 2>&1 | grep -q -- "--swap-space"; then ODYSSEUS_VLLM_SUPPORTS_SWAP=1; fi'
                )
                runner_lines.append("fi")
                runner_lines.append(
                    'if [ -z "$ODYSSEUS_PREFLIGHT_EXIT" ] && [ "${ODYSSEUS_VLLM_SUPPORTS_SWAP:-0}" = "1" ] && ! printf "%s" "$ODYSSEUS_SERVE_CMD" | grep -q -- "--swap-space"; then'
                )
                runner_lines.append(
                    '  echo "[odysseus] Setting vLLM --swap-space 0 so the runtime does not reserve CPU swap per GPU."'
                )
                runner_lines.append(
                    '  ODYSSEUS_SERVE_CMD="${ODYSSEUS_SERVE_CMD} --swap-space 0"'
                )
                runner_lines.append("fi")
                runner_lines.append(
                    'if [ -z "$ODYSSEUS_PREFLIGHT_EXIT" ] && [ "${ODYSSEUS_VLLM_SUPPORTS_SWAP:-0}" != "1" ]; then'
                )
                runner_lines.append(
                    '  if printf "%s" "$ODYSSEUS_SERVE_CMD" | grep -q -- "--swap-space"; then'
                )
                runner_lines.append(
                    '    echo "[odysseus] vLLM serve does not expose --swap-space; removing the flag and patching the runtime default to 0."'
                )
                runner_lines.append(
                    '    ODYSSEUS_SERVE_CMD="$(python3 - "$ODYSSEUS_SERVE_CMD" <<\'PY\''
                )
                runner_lines.append("import shlex, sys")
                runner_lines.append("parts = shlex.split(sys.argv[1])")
                ...
                runner_lines.append(
                    "def _odysseus_create_engine_config(self, *args, **kwargs):"
                )
                ...
                runner_lines.append(
                    "EngineArgs.create_engine_config = _odysseus_create_engine_config"
                )
                runner_lines.append(
                    "AsyncEngineArgs.create_engine_config = _odysseus_create_engine_config"
                )
                ...
                runner_lines.append(')"')
                runner_lines.append(
                    '  echo "[odysseus] Patched vLLM internal swap_space default to 0 for this runtime."'
                )
                runner_lines.append("fi")
                ...
                runner_lines.append(
                    '  echo "ERROR: SGLang is installed but failed to import."'
                )
                runner_lines.append('  printf "%s\\n" "$ODYSSEUS_SGLANG_IMPORT_ERROR"')
                runner_lines.append("  ODYSSEUS_PREFLIGHT_EXIT=127")
                runner_lines.append("fi")
            elif (
                "scripts/diffusion_server.py" in req.cmd
                or ".diffusion_server.py" in req.cmd
            ):
                runner_lines.append('export PATH="$HOME/.local/bin:$PATH"')
                runner_lines.append(
            ...
            if not handled_ollama_serve and re.search(
                r"\bdocker\s+exec\s+(?:ollama-rocm|ollama-test)\s+ollama\s+show\b",
                req.cmd or "",
            ):
                ...
                runner_lines.append(
                    '  echo "[odysseus] Ollama sidecar model is available; keeping Cookbook task attached to the persistent Ollama daemon."'
                )
    ...
    async def _probe_gpu_device_processes(
        host: str | None, ssh_port: str | None
    ) -> list[dict]:
        pid_cmd = (
    ...
    async def _probe_amd_sysfs(host: str | None, ssh_port: str | None) -> list[dict]:
        ...
        _amd_runtime = (
            (rt_out or "").strip().splitlines()[-1:][0].strip() if rt_out else "rocm"
        )
        ...
        for entry in out.split():
            ...
            if vendor != "0x1002":
                continue
            vram_raw = await _gpu_read_file(
                f"{base}/mem_info_vram_total", host, ssh_port
            )
            ...
            gtt_used_raw = await _gpu_read_file(
                f"{base}/mem_info_gtt_used", host, ssh_port
            )
            gtt_used_mb = (
    ...
    async def list_gpus(
        ...
        if not host and sys.platform == "darwin":
            try:
                ...
                if (
                    backend in {"metal", "mps", "apple"}
                    and info.get("gpu_count", 0) > 0
                ):
                    total_mb = int(
    ...
    async def save_cookbook_state(request: Request):
        ...
        try:
            ...
            try:
                if _cookbook_state_path.exists():
                    on_disk = json.loads(
                        _cookbook_state_path.read_text(encoding="utf-8")
                    )
                else:
                    on_disk = {}
            except Exception:
            ...
            incoming_removed = (
                data.get("removedTasks")
                if isinstance(data.get("removedTasks"), dict)
                else {}
            )
            disk_removed = (
                on_disk.get("removedTasks")
                if isinstance(on_disk, dict)
                and isinstance(on_disk.get("removedTasks"), dict)
                else {}
            )
            ...
            for _it in incoming_tasks:
                if (
                    (not isinstance(_it, dict))
                    or _it.get("type") != "download"
                    or _it.get("status") != "done"
                ):
                    continue
                _out = _it.get("output") or ""
                if (
                    ("DOWNLOAD_OK" in _out)
                    or ("DOWNLOAD_FAILED" in _out)
                    or ("/snapshots/" in _out)
                ):
                    continue
                ...
                if _shards:
                    ...
                    if int(_n) < int(_tot):
                        logger.info(
                            f"cookbook state POST: rejecting stale done for {_it.get('sessionId')} "
    ...
    def _maybe_sweep_orphans(tasks: list, state: dict) -> None:
        ...
        try:
            threading.Thread(
                target=_run_sweep, daemon=True, name="orphan-sweep"
            ).start()
        except Exception as _e:
    ...
    def _sync_sweep_orphans(tasks: list, state: dict) -> None:
        ...
        logger.info(
            f"orphan sweep starting: {len(servers) if isinstance(servers, list) else 0} server(s), known_sids={len([t for t in tasks if isinstance(t, dict) and t.get('sessionId')])}"
        )
        ...
        for srv in servers:
            ...
            for line in (ls.stdout or "").splitlines():
                ...
                try:
                    cap = subprocess.run(
                        ssh_base
                        + [host, "tmux", "capture-pane", "-t", sid, "-p", "-S", "-300"],
                        timeout=6,
                        capture_output=True,
                        text=True,
                    )
            ...
            async with httpx.AsyncClient(timeout=15, follow_redirects=True) as client:
                ...
                if resp.status_code != 200:
                    return {
                        "ok": False,
                        "files": [],
                        "error": f"HF API HTTP {resp.status_code}",
                    }
                data = resp.json()
        except Exception:
    ...
    _OLLAMA_FALLBACK_LIBRARY = [
        {
            "name": "qwen2.5",
            "description": "Qwen2.5 series — strong general/coding model from Alibaba.",
            "sizes": ["0.5b", "1.5b", "3b", "7b", "14b", "32b", "72b"],
        },
        {
            "name": "qwen2.5-coder",
            "description": "Code-specialized Qwen2.5 family.",
            "sizes": ["0.5b", "1.5b", "3b", "7b", "14b", "32b"],
        },
        {
            "name": "qwen3",
            "description": "Qwen3 — newer Alibaba family with hybrid reasoning.",
            "sizes": ["0.6b", "1.7b", "4b", "8b", "14b", "32b"],
        },
        {
            "name": "llama3.2",
            "description": "Meta Llama 3.2 instruct (and tiny / vision variants).",
            "sizes": ["1b", "3b", "11b", "90b"],
        },
        {
            "name": "llama3.1",
            "description": "Meta Llama 3.1 instruct.",
            "sizes": ["8b", "70b", "405b"],
        },
        {
            "name": "llama3.3",
        ...
        {
            "name": "gemma3",
            "description": "Google Gemma 3 — multimodal capable open-weights.",
            "sizes": ["1b", "4b", "12b", "27b"],
        },
        {
            "name": "gemma2",
        ...
        {
            "name": "mistral",
            "description": "Mistral 7B instruct — small, fast generalist.",
            "sizes": ["7b"],
        },
        {
            "name": "mistral-nemo",
        ...
        {
            "name": "mistral-small",
            "description": "Mistral Small 22B / 24B instruct.",
            "sizes": ["22b", "24b"],
        },
        {
            "name": "mixtral",
            "description": "Mistral MoE 8x7B / 8x22B.",
            "sizes": ["8x7b", "8x22b"],
        },
        {
            "name": "phi3",
            "description": "Microsoft Phi-3 small / medium.",
            "sizes": ["mini", "medium"],
        },
        {"name": "phi4", "description": "Microsoft Phi-4 14B.", "sizes": ["14b"]},
        {
            "name": "deepseek-r1",
            "description": "DeepSeek R1 reasoning model (distilled variants).",
            "sizes": ["1.5b", "7b", "8b", "14b", "32b", "70b"],
        },
        {
            "name": "deepseek-v3",
            "description": "DeepSeek V3 MoE 671B (huge — needs serious VRAM).",
            "sizes": ["671b"],
        },
        {
            "name": "codellama",
            "description": "Meta Code Llama instruct family.",
            "sizes": ["7b", "13b", "34b", "70b"],
        },
        {
            "name": "starcoder2",
            "description": "BigCode StarCoder2 — code completion.",
            "sizes": ["3b", "7b", "15b"],
        },
        {
            "name": "deepseek-coder-v2",
            "description": "DeepSeek Coder V2 — code MoE.",
            "sizes": ["16b", "236b"],
        },
        {
            "name": "nomic-embed-text",
            "description": "Embedding model — text vector encoder.",
            "sizes": ["latest"],
        },
        {
            "name": "mxbai-embed-large",
            "description": "Embedding model — Mixedbread large.",
            "sizes": ["latest"],
        },
        {
            "name": "llava",
        ...
        {
            "name": "command-r",
            "description": "Cohere Command R — RAG-oriented.",
            "sizes": ["35b"],
        },
        {
            "name": "command-r-plus",
            "description": "Cohere Command R+ — larger RAG model.",
            "sizes": ["104b"],
        },
        {"name": "qwq", "description": "Qwen QwQ reasoning preview.", "sizes": ["32b"]},
        {
            "name": "smollm2",
            "description": "HuggingFaceTB SmolLM2 — tiny capable models.",
            "sizes": ["135m", "360m", "1.7b"],
        },
        {
            "name": "granite3.1-dense",
            "description": "IBM Granite 3.1 dense instruct.",
            "sizes": ["2b", "8b"],
        },
        {"name": "nemotron", "description": "NVIDIA Nemotron 70B.", "sizes": ["70b"]},
        {
            "name": "olmo2",
            "description": "AI2 OLMo 2 open-weights.",
            "sizes": ["7b", "13b"],
        },
    ]
    ...
    async def ollama_library(
        refresh: int = 0, request: Request = None, owner: str = Depends(require_user)
    ):
        ...
        if (
            refresh
            or (now - _ollama_library_cache["fetched_at"]) > TTL
            or not _ollama_library_cache["models"]
        ):
            models: list[dict] = []
            err = None
            try:
                    ...
                    for bm in block_re.finditer(html):
                        ...
                        for s in sizes_raw:
                            ...
                            if s_low not in sizes:
                                sizes.append(s_low)
                        models.append(
                            {"name": name, "description": desc, "sizes": sizes}
                        )
                        if len(models) >= 80:
                            break
                else:
    ...
    async def vllm_recipe(repo: str, refresh: int = 0):
        ...
        normalized = {
            ...
            "hardware_overrides": {
                ...
                for hw, ov in hw_overrides.items()
                if isinstance(ov, dict)
            },
            "strategy_overrides": {
                strat: dict(ov or {})
                for strat, ov in strat_overrides.items()
                if isinstance(ov, dict)
            },
    ...
    def _cookbook_tasks_status_sync():
        ...
        def _download_cache_complete(
            ...
            try:
                if remote_host:
                    ...
                    proc = subprocess.run(
                        ssh_base + [remote_host, shell_cmd],
                        timeout=12,
                        capture_output=True,
                    )
                else:
        ...
        def _download_cache_incomplete(
            repo_id: str,
            remote_host: str = "",
            ssh_port: str = "",
            cache_root: str = "",
        ...
        for task in tasks:
            ...
            if local_win_task:
                ...
                try:
                    if log_path.exists():
                        ...
                        lines = [
                            l.strip() for l in full_snapshot.split("\n") if l.strip()
                        ]
                        downloading_lines = [
                            l for l in lines if l.startswith("Downloading")
                        ]
                ...
                if _task_status in {
                    ...
                    try:
                        alive = subprocess.run(
                            check_cmd, timeout=4, capture_output=True
                        )
                    ...
                    if is_alive:
                        try:
                            cap = subprocess.run(
                                capture_cmd, timeout=4, capture_output=True, text=True
                            )
                            if cap.returncode == 0:
                                full_snapshot = cap.stdout.strip()
                                lines = [
                                    l.strip()
                                    for l in full_snapshot.split("\n")
                                    if l.strip()
                                ]
                                downloading_lines = [
                                    l for l in lines if l.startswith("Downloading")
            ...
            download_has_failed = (
                task_type == "download" and "DOWNLOAD_FAILED" in full_snapshot
            )
            download_has_incomplete_evidence = task_type == "download" and (
                ".incomplete" in full_snapshot
                or bool(
                    re.search(
                        r"model-\d+-of-\d+\.[A-Za-z0-9_.-]+:\s+(?:[0-9]|[1-8][0-9])%",
                        full_snapshot,
                    )
                )
                or _download_cache_incomplete(
            ...
            if is_alive or (local_win_task and full_snapshot):
                ...
                marker = (
                    classify_dead_download(full_snapshot)
            ...
            if (
                diagnosis
                and status in {"running", "unknown", "stopped"}
                and phase_info.get("status") != "ready"
            ):
                status = "error"
            if download_zero_files:
                diagnosis = {
                    "message": "No matching files were downloaded. The model repo or filename/quant pattern may be wrong (for example a ':Q4_K_M' tag that does not exist in the repo). Check the repo and the include/quant pattern."
                }
            ...
            results.append(
                {
                    ...
                    "status": status,
                    "progress": serve_phase
                    if task_type == "serve"
                    else progress_text[:120],
                    "phase": serve_phase,
                    "diagnosis": diagnosis,
