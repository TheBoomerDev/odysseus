...
def _skill_test_task(skill: dict) -> str:
    ...
    if not isinstance(skill, dict):
        skill = {}
    ctx = (
        skill.get("when_to_use") or skill.get("description") or skill.get("name") or ""
    ).strip()
    return (
...
async def _eval_skill_run(
    ...
    def _parse(raw: str):
        ...
        text = _re.sub(
            r"<think(?:ing)?>[\s\S]*?</think(?:ing)?>", "", text, flags=_re.I
        )
        ...
        v = (
            str(data.get("verdict", "")).lower().strip()
        ...
        return {
            ...
            "issues": [
                str(x)[:200] for x in (data.get("issues") or []) if str(x).strip()
            ][:8],
        }
    ...
    if last_err is not None and not last_text:
        return {
            "verdict": "unknown",
            "confidence": 0,
            "summary": f"Evaluator call failed: {last_err}",
            "issues": [],
        }
    return {
        "verdict": "unknown",
        "confidence": 0,
        "summary": "Evaluator returned unparseable output.",
        "issues": [],
        "raw": last_text[:300],
    }


async def _eval_skill_necessity(
    skill_md: str, others: list, url: str, model: str, headers: Optional[dict]
) -> Optional[dict]:
    ...
    catalog = (
        "\n".join(f"- {o.get('name')}: {o.get('description', '')}" for o in others)
        or "(no other skills)"
    )
    sys_prompt = (
    ...
    try:
        ...
        return None
    text = _re.sub(
        r"<think(?:ing)?>[\s\S]*?</think(?:ing)?>", "", (raw or ""), flags=_re.I
    )
    text = _re.sub(r"<think(?:ing)?>[\s\S]*$", "", text, flags=_re.I).strip()
    data = None
    a, b = text.find("{"), text.rfind("}")
    ...
    return {
        ...
        "redundant_with": [
            str(x)[:80] for x in (data.get("redundant_with") or []) if str(x).strip()
        ][:6],
...
async def _eval_skill_retrieval_precision(
    skill_md: str, others: list, url: str, model: str, headers: Optional[dict]
) -> Optional[dict]:
    ...
    catalog = (
        "\n".join(f"- {o.get('name')}: {o.get('description', '')}" for o in others[:80])
        or "(no other skills)"
    )
    sys_prompt = (
    ...
    return {
        ...
        "issues": [str(x)[:220] for x in (data.get("issues") or []) if str(x).strip()][
            :6
        ],
    }
...
async def _run_skill_test_job(
    key, name, md, task, url, model, headers, owner, skills_manager=None
):
    ...
    messages = [
        {
            "role": "system",
            "content": "You are TESTING a skill. Below is a reusable skill (a procedure). Follow it "
    ...
    try:
        job["verdict"] = await _eval_skill_run(
            md, task, "".join(transcript), url, model, headers
        )
    except Exception as e:
        job["verdict"] = {
            "verdict": "unknown",
...
def _skill_duplicate_blocker(skills_manager, name: str, owner) -> Optional[str]:
    ...
    def _tokens(sk: dict) -> set[str]:
        ...
        return {
            t
            for t in _re.split(r"[^a-z0-9]+", text)
...
def _audit_generic_blocker(
    skill: Optional[dict], necessity: Optional[dict], verdict_data: Optional[dict]
) -> Optional[str]:
...
def _audit_finalize_status(
    skills_manager,
    name: str,
    owner,
    verdict: str,
    confidence: Optional[float],
    necessity: Optional[dict] = None,
    ...
    necessary = True
    current = next(
        (s for s in skills_manager.load(owner=owner) if s.get("name") == name), None
    )
    ...
    if generic_reason:
        ...
        try:
            ...
            pass
    duplicate_of = (
        _skill_duplicate_blocker(skills_manager, name, owner)
        if verdict == "pass"
        else None
    )
    if duplicate_of:
    ...
    status = (
        "published"
        if (auto_publish and necessary and verdict == "pass" and c >= min_conf)
        else "draft"
    )
    try:
...
async def _run_skill_test_once(md: str, task: str, url, model, headers, owner) -> tuple:
    ...
    messages = [
        {
            "role": "system",
            "content": "You are TESTING a skill. Follow this skill's procedure to complete the task "
...
async def _improve_skill_md(
    skill_md: str, verdict: dict, transcript: str, url, model, headers
):
...
async def _audit_one_skill(
    skills_manager, skill, url, model, headers, teacher, owner, log
) -> dict:
    ...
    try:
        ...
        if nec is not None:
            skills_manager.set_necessity(
                name,
                nec.get("necessary", True),
                nec.get("redundant_with"),
                nec.get("reason"),
                owner=owner,
            )
    ...
    if (
        generic_reason
        or duplicate_of
        or (isinstance(nec, dict) and nec.get("necessary") is False)
    ):
        reason = generic_reason or (
            f"Lower-priority duplicate of {duplicate_of}"
            if duplicate_of
            else str((nec or {}).get("reason") or "Unnecessary skill")
        )
        try:
            skills_manager.update_skill(
                name, {"status": "draft", "confidence": 0.35}, owner=owner
            )
            skills_manager.set_audit(
                name, "skipped", by_teacher=False, worker_model=model, owner=owner
            )
            if duplicate_of:
                skills_manager.set_necessity(
    ...
    try:
        if _should_check_retrieval_precision(skill):
            ...
            if rp and not rp.get("ok"):
                ...
                log(
                    f"{name}: narrowing retrieval metadata — {(rp.get('summary') or issues[0])[:80]}"
                )
                fixed = await _improve_skill_md(
                    md,
                    {
                        ...
                        or "Retrieval metadata is too broad.",
                        "issues": issues,
                    },
                ...
                if (
                    fixed
                    and fixed.strip() != md.strip()
                    and _apply_skill_md(skills_manager, name, fixed, owner)
                ):
                    md = fixed
                    refreshed = next(
                        (
    ...
    if v in ("unknown", "inconclusive"):
        skills_manager.set_audit(
            name, "inconclusive", by_teacher=False, worker_model=model, owner=owner
        )
        status = _audit_finalize_status(
            skills_manager,
            name,
    ...
    if (
        new_md
        and new_md.strip() != md.strip()
        and _apply_skill_md(skills_manager, name, new_md, owner)
    ):
        md = new_md
        transcript, verdict = await _run_skill_test_once(
            md, task, url, model, headers, owner
        ...
        if v == "pass":
            ...
            refreshed = next(
                (s for s in skills_manager.load(owner=owner) if s.get("name") == name),
                None,
            )
            status = _audit_finalize_status(
                skills_manager,
    ...
    if (
        ...
        and teacher[0]
        and teacher[1]
        and (teacher[1] != model or teacher[0] != url)
    ):
        teacher_ran = True
        t_url, t_model, t_headers = teacher
        log(f"{name}: teacher {t_model} rewriting the skill…")
        t_md = await _improve_skill_md(
            md, verdict, transcript, t_url, t_model, t_headers
        )
        if (
            t_md
            and t_md.strip() != md.strip()
            and _apply_skill_md(skills_manager, name, t_md, owner)
        ):
            md = t_md
...
def _resolve_audit_models(owner=None):
    ...
    try:
        ...
        if _avail and model not in _avail:
            ...
            model = (
                next(
                    (a for a in _avail if _os.path.basename(a.rstrip("/")) == _base),
                    None,
                )
                or _avail[0]
            )
...
async def run_scheduled_skill_audit(
    skills_manager: SkillsManager, owner: Optional[str] = None, max_skills: int = 8
) -> dict:
    ...
    skills.sort(
        key=lambda s: s.get("audited_at") if s.get("audited_at") is not None else -1.0
    )
    ...
    _skill_audit_jobs[key] = {
        ...
        "results": [],
        "log": [
            f"Nightly audit of {len(names)} least-recently-checked skill(s) with {model}"
    ...
    logger.info(
        f"Scheduled skill audit starting: {len(names)} skill(s) (owner={owner or 'all'})"
    )
...
def setup_skills_routes(skills_manager: SkillsManager) -> APIRouter:
    ...
    async def list_builtin_skills(request: Request):
        ...
        def _clean(raw: str) -> str:
            ...
            s = re.sub(
                r"```.*?```", "", s, flags=re.S
            )  # drop code fences (incl. inline ```name```)
            s = re.sub(r"\s+", " ", s).strip()
            s = re.sub(
                r"^[-–—:\s]+", "", s
            )  # drop leftover "- — " / ": " bullet prefix
            return s[:240]
    ...
    async def test_skill(request: Request, skill_id: str):
        ...
        if not url or not model:
            raise HTTPException(
                400, "No model configured — set a Default or Utility model in Settings."
            )
        ...
        if isinstance(requested_names, list):
            ...
            for raw in requested_names:
                ...
                if not nm or nm in seen or nm not in by_name:
                    continue
                if (
                    scope not in ("all", "selected")
                    and (by_name[nm].get("status") or "draft") == "published"
                ):
                    continue
    ...
    async def save_skill_markdown(request: Request, skill_id: str):
        ...
        ok = skills_manager.update_skill(
            match.get("name"),
            {
                "name": sk.name,
                "description": sk.description,
