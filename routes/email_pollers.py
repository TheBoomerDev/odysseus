...
async def _auto_summarize_pass(
    days_back: int = 1, account_id: str | None = None, progress_cb=None
) -> str:
    ...
    if account_id is None:
        ...
        for idx, aid in enumerate(ids, start=1):
            try:
                ...
                result = await _auto_summarize_pass_single(
                    days_back=days_back, account_id=aid, progress_cb=progress_cb
                )
        ...
        return "\n".join(outs)
    return await _auto_summarize_pass_single(
        days_back=days_back, account_id=account_id, progress_cb=progress_cb
    )
        ...
        if auto_tag or auto_spam:
            if account_owner:
                _tag_existing = {
                    r[0]
                    for r in _c.execute(
                        "SELECT message_id FROM email_tags WHERE owner=?",
                        (account_owner,),
                    ).fetchall()
                }
            else:
            ...
            try:
                ...
                if need_class:
                    try:
                        ...
                        if not resp.ok:
                            logger.warning(
                                f"Auto-classify {uid.decode() if isinstance(uid, bytes) else str(uid)} HTTP {resp.status_code}: {resp.text[:200]}"
                            )
                        else:
                            rdata = resp.json()
                            m = (rdata.get("choices") or [{}])[0].get("message", {})
                            ...
                            if parsed is not None:
                                ...
                                tags = [
                                    "marketing" if t == "promo" else t for t in tags
                                ]
