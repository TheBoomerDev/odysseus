    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                ["a"],
            ),  # name precedence over model
            (
                {"models": [{"name": 123}, {"model": None}, {"name": "ok"}]},
                ["ok"],
...
class TestProbeSingleModel:
    ...
    def test_completion_probe_uses_llm_verify(
        self, monkeypatch, base, api_key, model_id
    ):
        _patch_resolve(monkeypatch)
        marker = object()
        captured = {}
