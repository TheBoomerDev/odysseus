...
@pytest.mark.parametrize(
    "key", ["LD_PRELOAD", "NODE_OPTIONS", "PYTHONPATH", "DYLD_INSERT_LIBRARIES", "PATH"]
)
...
def test_validator_allows_safe_allowlisted_server():
    assert (
        _validate_mcp_command("mcp-server-demo", ["--port", "3000"], {"FOO": "bar"})
        is None
    )
...
def test_add_rejects_rce_with_no_db_write_and_no_connect(monkeypatch):
    ...
    try:
        assert db.query(McpServer).count() == 0, (
            "rejected add must not persist an enabled row"
        )
    finally:
        db.close()
