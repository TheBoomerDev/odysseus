...
def test_vllm_route_strips_swap_space_when_runtime_rejects_it():
    ...
    assert (
        "Setting vLLM --swap-space 0 so the runtime does not reserve CPU swap per GPU."
        in text
    )
    assert (
        "vLLM serve does not expose --swap-space; removing the flag and patching the runtime default to 0."
        in text
    )
...
def test_local_windows_platform_comes_from_backend_host_state():
    ...
    assert (
        "s.platform = _envState.hostPlatform || _envState.platform || '';" not in text
    )
