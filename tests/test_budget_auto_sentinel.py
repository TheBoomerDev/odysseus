...
def test_auto_scales_on_a_known_window():
    assert compute_input_token_budget(DEFAULT_BUDGET, 131072, explicit=False) == int(
        131072 * 0.85
    )
...
def test_auto_stays_conservative_on_unknown_window():
    ...
    assert (
        compute_input_token_budget(DEFAULT_BUDGET, 0, explicit=False) == DEFAULT_BUDGET
    )
...
def test_nondefault_value_is_an_explicit_cap():
    ...
    assert (
        compute_input_token_budget(200000, 32000, explicit=True) == 32000
    )  # clamped to window
...
def test_get_context_length_known_surfaces_endpoint_proven_vs_fallback():
    ...
    mc._context_cache.clear()
    with patch.object(
        mc, "_query_context_length", return_value=(mc.DEFAULT_CONTEXT, False)
    ):
...
def test_no_arg_caller_scales_from_discovered_window_not_6000():
    ...
    assert compute_input_token_budget(DEFAULT_BUDGET, ctx, explicit=False) == int(
        131072 * 0.85
    )
