...
def test_default_scales_to_context_window():
    ...
    assert compute_input_token_budget(6000, 128000, explicit=False) == int(
        128000 * 0.85
    )
...
def test_default_capped_at_hard_max_for_huge_windows():
    assert (
        compute_input_token_budget(6000, 1_000_000, explicit=False) == DEFAULT_HARD_MAX
    )
...
def test_invalid_numeric_inputs_fall_back_cleanly():
    ...
    assert compute_input_token_budget("bad", 128000, explicit=True) == int(
        128000 * 0.85
    )
...
def test_custom_hard_max_overrides_default_in_auto_branch():
    ...
    assert compute_input_token_budget(
        6000, 1_000_000, explicit=False, hard_max=900_000
    ) == int(1_000_000 * 0.85)
...
def test_custom_hard_max_lowers_default_for_cost_paranoid_setups():
    ...
    assert compute_input_token_budget(6000, 128_000, explicit=False) == int(
        128_000 * 0.85
    )
    # Same ctx, ceiling lowered to 50K -> capped at 50K instead
    assert (
        compute_input_token_budget(6000, 128_000, explicit=False, hard_max=50_000)
        == 50_000
    )
...
def test_hard_max_has_no_effect_on_explicit_branch():
    ...
    assert (
        compute_input_token_budget(900_000, 1_000_000, explicit=True, hard_max=100_000)
        == 900_000
    )
...
def test_agent_loop_reads_hard_max_setting(tmp_path, monkeypatch):
    ...
    assert (
        settings.get_setting("agent_input_token_hard_max", DEFAULT_HARD_MAX) == 750_000
    )
