...
class TestDoRoutePrompt:
    ...
    async def test_basic_routing(self):
        ...
        result = await do_route_prompt(
            json.dumps({"prompt": "write code for a login page"})
        )
    ...
    async def test_with_priority(self):
        ...
        result = await do_route_prompt(
            json.dumps({"prompt": "test the API", "priority": "cost"})
        )
        assert "error" not in result
    ...
    async def test_invalid_priority(self):
        ...
        result = await do_route_prompt(
            json.dumps({"prompt": "hello", "priority": "invalid"})
        )
        assert "error" in result
        assert result["exit_code"] == 1
...
class TestDoCsuiteQuery:
    ...
    async def test_unknown_role(self):
        ...
        result = await do_csuite_query(
            json.dumps({"role": "nonexistent", "question": "test"})
        )
        assert "error" in result
...
class TestDoDecomposeGoal:
    ...
    async def test_basic_decompose(self):
        ...
        result = await do_decompose_goal(
            json.dumps({"goal": "migrate database to postgres"})
        )
        assert "error" not in result
