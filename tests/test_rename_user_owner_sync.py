...
def test_rename_updates_personal_rag_upload_owner(rename_endpoint, monkeypatch):
    ...
    rag_calls = []
    rag = SimpleNamespace(
        rename_owner=lambda old, new, path_map=None, path_prefixes=None: (
            rag_calls.append(
...
def test_rejected_rename_does_not_mutate_files(monkeypatch, tmp_path):
    ...
    assert json.loads(rp.read_text())["owner"] == "alice", (
        "research owner mutated after rejected rename"
    )
    assert json.loads(mem.read_text())[0]["owner"] == "alice", (
        "memory owner mutated after rejected rename"
    )
    assert "owner: alice" in (skill_dir / "SKILL.md").read_text(), (
