...
def test_legacy_collection_backfills_custom_only_lane(monkeypatch):
    ...
    assert (
        len(
            fake.collections["odysseus_memories_custom"].rows["legacy-memory"][
                "embedding"
            ]
        )
        == 768
...
def test_legacy_migration_resumes_partial_lane_backfill(monkeypatch):
    ...
    partial = fake.get_or_create_collection(
    ...
    assert set(fake.collections["odysseus_memories_fastembed"].get()["ids"]) == {
        "legacy-1",
        "legacy-2",
    }
