...
def test_timed_non_positive_end_clamped_to_one_hour():
    ...
    assert _ensure_positive_duration(start, start, False) == datetime(
        2026, 6, 20, 10, 0
    )
    # reversed end (dtend < dtstart) is also normalized
    earlier = datetime(2026, 6, 20, 8, 0)
    assert _ensure_positive_duration(start, earlier, False) == datetime(
        2026, 6, 20, 10, 0
    )
...
def test_single_day_all_day_event_with_same_date_end_appears_in_list():
    ...
    res = asyncio.run(
        eps["import"](
            _request(),
            file=_FakeUpload(_ics("20260620", "20260620")),
            calendar_name="A",
        )
    )
...
def test_normal_multi_day_all_day_event_still_appears():
    ...
    res = asyncio.run(
        eps["import"](
            _request(),
            file=_FakeUpload(_ics("20260710", "20260711")),
            calendar_name="B",
        )
    )
    assert res["imported"] == 1
...
def test_reimport_repairs_legacy_zero_duration_row():
    ...
    res = asyncio.run(
        eps["import"](
            _request(),
            file=_FakeUpload(_ics("20260801", "20260801")),
            calendar_name="C",
        )
    )
